...
var atoms = atomNames(map[string]string{
	...
	"cpil":    "compilation",
	"disk":    "disc",
	// SONARLY(W2): iTunes content rating (0=clean, 1=explicit, 2=advisory);
	// absent upstream, which silently dropped the explicit flag for m4a.
	"rtng": "rating",
})
...
func (m metadataMP4) readAtomData(r io.ReadSeeker, name string, size uint32, processedData []string) error {
	...
	case "uint8":
		// SONARLY(W2): tmpo (BPM) is a 2-byte big-endian int; upstream read
		// only 1 byte and misreported every BPM >= 256... and any value with a
		// zero high byte (e.g. 128) as 0.
		if name == "tmpo" {
			if len(b) < 2 {
				return fmt.Errorf("invalid encoding: expected at least %d bytes, for tempo tag data, got %d", 2, len(b))
			}
			data = getInt(b[:2])
			break
		}
	...
	// SONARLY(W1): repeated atoms (e.g. multiple ©ART / ©gen / ---- data
	// atoms) accumulate into []string instead of overwriting (upstream:
	// last-wins, losing all but one artist/genre/MBID).
	if prev, exists := m.data[name]; exists {
		switch pv := prev.(type) {
		case string:
			if s, ok := data.(string); ok {
				m.data[name] = []string{pv, s}
			} else {
				m.data[name] = data
			}
		case []string:
			if s, ok := data.(string); ok {
				m.data[name] = append(pv, s)
			} else {
				m.data[name] = data
			}
		default:
...
func readCustomAtom(r io.ReadSeeker, size uint32) (_ string, data []string, _ error) {
	...
	for size > 8 {
		...
		case "data":
			// SONARLY(W1): the data atom payload is [type u32][locale u32]
			// followed by the value; upstream skipped only the type, leaving
			// the 4 locale bytes prefixed to every freeform value.
...
func (m metadataMP4) getString(n []string) string {
	for _, k := range n {
		if x, ok := m.data[k]; ok {
			// SONARLY(W1): multi-value atoms are []string; scalars use the first.
			switch v := x.(type) {
			case string:
				return v
			case []string:
