...
- OpenSubsonic API: `getAlbumInfo`/`getAlbumInfo2` now return the standard `albumInfo` element (the non-standard `albumInfo2` key crashed Music Assistant album syncs with `KeyError: 'albumInfo'`), and the previously unimplemented `getAlbumInfo` endpoint is available as an alias. Bogus cover-art id values were dropped from `smallImageUrl`/`largeImageUrl`.
- OpenSubsonic API: `getLyrics` now returns the schema-shaped `{value, artist, title}` object instead of a bare string, which crashed Music Assistant track syncs.
- OpenSubsonic API: `getPodcasts`, `getNewestPodcasts`, and `getInternetRadioStations` now return valid empty collections instead of 404, so clients that sync podcasts/radio stations (Music Assistant enables both by default) succeed with zero items.
...
[Unreleased]: https://github.com/miquelrosell99/sonarly/compare/v0.7.1...HEAD
