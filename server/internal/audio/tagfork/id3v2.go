...
func readID3v2Frames(r io.Reader, offset uint, h *id3v2Header) (map[string]interface{}, error) {
	...
	for offset < h.Size {
		...
		case name[0] == 'T':
			...
			// SONARLY(W1): keep multi-value frames as []string; single-value
			// frames stay plain strings (upstream shape).
			switch len(txt) {
			case 0:
				result[rawName] = ""
			case 1:
				result[rawName] = txt[0]
			default:
				result[rawName] = txt
			}
		...
		case name[0] == 'W':
			...
			// SONARLY(W1): same multi-value handling as T frames.
			switch len(txt) {
			case 0:
				result[rawName] = ""
			case 1:
