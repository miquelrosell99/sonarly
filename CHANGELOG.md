...
- Deep links into playback: `/now-playing/<playlist|album>/<contextId>/<songId>` loads the context as the queue, starts at the song, and opens Now Playing over the context page. The player tracks the queue's origin (set by playlist/album play actions) and Now Playing shows a copy-link button when the origin is known. Clicking a track in a playlist/album now queues the whole context from that track instead of the single song.
