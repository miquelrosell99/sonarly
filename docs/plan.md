...
- [x] P9 — OpenSubsonic adapter full parity ✅ (`a686592`): **all 62 quirks done** — P9a browsing+retrieval (24 endpoints), P9b starring/now-playing/playlists/bookmarks (15 endpoints) + native favorites/ratings/half-ratings, preferences allowlist (Q8 fix), avatar stub. 498 tests total.
...
- [x] **P10b — v1→v2 data migration + dual-run** ✅ (`ddba131`, **corrected 2026-09-26**): the original run pointed at the compose *fallback* dir (20 files) instead of the real `LIBRARY_MUSIC` mount (`/srv/.../Música`, 238 GB, 7,421 files = catalog match) — the "stale catalog" conclusion was a path artifact, **no mass-deactivation awaits at cutover**. Original evidence retained: streams byte-identical, playlists/history/interactions byte-identical, rollback runbook + additive-schema analysis in `docs/v2-p10b-dualrun-report.md`. Corrected rerun against the real path: see plan entry below.
