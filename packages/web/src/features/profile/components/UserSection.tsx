  ...
  return (
    <div ref={ref} className="relative mt-auto pt-4">
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        className="flex w-full items-center gap-3 rounded-md p-2 text-left transition hover:bg-surface-hover focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent"
        aria-haspopup="menu"
        aria-expanded={open}
      >
      ...
      {open && (
        <div
          role="menu"
          className="absolute bottom-full left-0 mb-2 w-48 rounded-md border border-rule bg-surface py-1 shadow-sm"
        >
          <button
            type="button"
            role="menuitem"
            onClick={() => { setOpen(false); onSettings(); }}
            className="block w-full px-4 py-2 text-left text-sm text-fg-primary transition hover:bg-surface-hover focus-visible:bg-surface-hover focus-visible:outline-none"
          >
            Settings
          </button>
          {user.isAdmin && (
