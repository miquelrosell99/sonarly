...
describe('management settings endpoints', () => {
  ...
  it('updates the organize pattern', async () => {
    const newPattern = '{albumArtist}/{album}/{title}{ext}';
  ...
  it('rejects invalid patterns', async () => {
    ...
    expect(JSON.parse(res.body).error).toContain('{ext}');
  });
  ...
  it('forbids non-admins', async () => {
    ...
    const patch = await app.inject({
      ...
      url: '/api/settings/media',
      cookies: { sessionId: regularCookie },
      payload: { organizePattern: '{artist}/{title}{ext}' },
    });
  ...
  it('reflects persisted pattern in organize preview', async () => {
    const newPattern = '{artist}/{title}{ext}';
    await app.inject({
      method: 'PATCH',
      url: '/api/settings/media',
      cookies: { sessionId: adminCookie },
