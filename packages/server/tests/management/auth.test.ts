...
describe('management auth endpoints', () => {
  ...
  it('returns me when authenticated', async () => {
    ...
    expect(res.statusCode).toBe(200);
    expect(JSON.parse(res.body).user).toMatchObject({ userId: 'admin-1', username: 'admin', isAdmin: true });
  });
  ...
  it('completes setup for an empty database', async () => {
    ...
    migrate(db);
    db.close();
    const emptyApp = await buildApp(emptyConfig);
    ...
    const setup = await emptyApp.inject({
      method: 'POST',
      url: '/api/setup',
      payload: { username: 'first', password: 'firstpass' },
    });
    expect(setup.statusCode).toBe(201);
    const body = JSON.parse(setup.body);
    expect(body.user).toMatchObject({ username: 'first', isAdmin: true });
