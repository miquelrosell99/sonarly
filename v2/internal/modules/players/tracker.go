...
//
//   - v1's tracker only saw OpenSubsonic client streams (the hook lived in
//     the subsonic retrieval path). v2 hooks the playback service itself,
...
//     entries; v1 keyed by user alone and the second device clobbered the
//     first.
//   - Artist/album names resolve at read time in two batched queries
//     (v1 ran two queries per recorded stream). Entries store ids only.
//   - The registry is in-memory and process-local, like v1: a restart
...
// TTL is how long an entry survives without a matching stream (v1's
// PLAYER_TTL_MS).
const TTL = 5 * time.Minute

// PlayerInfo is one live player entry (v1's PlayerInfo shape). The raw
// artist/album ids ride along unexported: the JSON keeps v1's names-only
...
// clientID derives the device key the way v1's getClientId did: the `c`
