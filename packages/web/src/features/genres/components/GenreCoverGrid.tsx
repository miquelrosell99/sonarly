...
interface GenreCoverGridProps {
  genreId: string;
}

export function GenreCoverGrid({ genreId }: GenreCoverGridProps) {
  ...
  useEffect(() => {
    let cancelled = false;
    setLoading(true);
    api<{ albums: Album[] }>(`/genres/${encodeURIComponent(genreId)}/albums?limit=4`)
      .then((res) => {
