...
describe('processIngestFolder', () => {
  ...
  beforeEach(() => {
    ...
    config = { PORT: 3000, DATA_DIR: '/data', LIBRARY_PATH: libraryPath, INGEST_PATH: ingestPath, ORGANIZE_PATTERN: '{artist}/{album}/{title}', SCAN_INTERVAL_MINUTES: 60, WATCHER_USE_POLLING: false, PUID: 1000, PGID: 1000, NODE_ENV: 'test', SESSION_SECRET: 'a'.repeat(32) };
  });
  ...
  it('removes empty source directories after importing', async () => {
    const sourceDir = join(ingestPath, 'uploads', 'batch-1');
    mkdirSync(sourceDir, { recursive: true });
    copyFileSync(fixturePath, join(sourceDir, 'sample.mp3'));
    const stats = await processIngestFolder(config, db);
    expect(stats.imported).toBe(1);
