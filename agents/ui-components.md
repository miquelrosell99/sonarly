...
| `EditEntityModal` | `components/EditEntityModal.tsx` | Tag-edit modal for songs/albums (single and multi-select), artists and smart playlists. Song payloads go through the capability-aware `buildSongTagsPatch` (`lib/songEditPatch.ts`) — genre names under `genre` on v1 and v2 alike; the FilePathInfo popover in the title renders only when the server exposes `filePath` (v1; hidden on v2); synced-lyrics counts go through `normalizeSyncedLyrics` so a v2 string value still counts lines. |
