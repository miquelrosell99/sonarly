...
describe('management settings endpoints', () => {
  ...
  describe('system tasks', () => {
    it('returns system tasks for admins', async () => {
      ...
      expect(body.tasks.map((t: { id: string }) => t.id)).toEqual(['periodic_scan', 'review_cleanup']);
    ...
    it('queues a periodic scan when run', async () => {
      const res = await app.inject({
        method: 'POST',
        url: '/api/settings/system-tasks/periodic_scan/run',
        cookies: { sessionId: adminCookie },
      });
    ...
    it('queues a review cleanup when run', async () => {
      const res = await app.inject({
        method: 'POST',
        url: '/api/settings/system-tasks/review_cleanup/run',
        cookies: { sessionId: adminCookie },
      });
      expect(res.statusCode).toBe(202);
    ...
    it('rejects invalid task ids', async () => {
      const res = await app.inject({
        method: 'POST',
        url: '/api/settings/system-tasks/unknown/run',
        cookies: { sessionId: adminCookie },
      });
      expect(res.statusCode).toBe(400);
    });
    ...
    it('requires authentication', async () => {
      const get = await app.inject({ method: 'GET', url: '/api/settings/system-tasks' });
      expect(get.statusCode).toBe(401);

      const post = await app.inject({ method: 'POST', url: '/api/settings/system-tasks/periodic_scan/run' });
