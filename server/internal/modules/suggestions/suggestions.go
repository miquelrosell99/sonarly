...
// the old features/suggestions/routes.ts ported query for query: LIKE
...
// allowedFields is the old ALLOWED_FIELDS.
...
// releaseTypeSeeds is the old RELEASE_TYPE_SEEDS: canonical casing first, the
...
// escapeLike ports the old LIKE escaping: % and _ get a backslash escape, and
...
// case-insensitively and sorted (the old buildGenrePaths-based branch).
...
// Handler wires the suggestion endpoint to HTTP (admin-gated, wire parity).
type Handler struct {
	svc *Service
	mw  *auth.Middleware
}
...
// clampLimit ports the old Math.min(Math.max(Number(limit) || 20, 1), 50):
