...
// Ports of the S2 spike's decision tables (spikes/streaming/decide_test.go).
...
func TestParseMaxBitRate(t *testing.T) {
	cases := []struct {
		...
		{"Infinity", 0, false},
		{"128abc", 0, false},
		{"999999", 0, false}, // S2 clamp scenario: out-of-range → ignored
	}
	for _, c := range cases {
...
func TestContentDisposition(t *testing.T) {
	...
	if got != want {
		t.Errorf("contentDisposition =\n%q\nwant\n%q", got, want)
	}
	// Parentheses stay raw (encodeURIComponent does not escape them).
	if got := contentDisposition("a (Remastered).flac"); got != `attachment; filename="a (Remastered).flac"; filename*=UTF-8''a%20(Remastered).flac` {
		t.Errorf("contentDisposition parentheses: %q", got)
	}
}
