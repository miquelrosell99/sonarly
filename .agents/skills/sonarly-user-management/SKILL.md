...
- `v2/internal/modules/auth/` — `store.go` (SQLite session store), `cookie.go` (signed cookie, wire-compatible with v1 so existing sessions survived the cutover), `throttle.go` (login throttling), `secret.go` (AES-256-GCM for `users.subsonic_password_encrypted`; key = SHA-256 of `SESSION_SECRET`), `apikeys.go` (SHA-256 key verification — **no management routes yet**, same half-built state as v1), `middleware.go` (session resolution).
- `v2/internal/modules/users/` — `service.go` (user CRUD, bcrypt cost 12, last-admin protections, session invalidation on role/password change), `routes.go` (login/logout/setup/me, profile, admin routes), `repository.go`, `preferences.go` (allowlisted PATCH keys, defaults merged under stored blob), `avatar.go` (file-backed avatars).
...
- Per-user *content* scoping (`user_libraries`) is **enforced in every read/stream path** — every catalog query filters by assigned libraries; songs with `library_id IS NULL` are admin-only. Treat any bypass as a security bug (v1's F1 decision made this a boundary and v2 enforces it from the start; the parity suites keep it closed).
...
- Public routes are a security decision, not a convenience — extending the unauthenticated set needs the same scrutiny as the v1 audit applied.
- Tests: `*_test.go` in `auth/` and `users/` (cookie interop, throttle, scoping matrix, preferences allowlist, admin gates). Run `go test ./... -count=1` from `v2/`.
