...
1. **Global / admin settings** — `settings` key-value table (server module `server/internal/modules/admin`, settings handlers).
   - Used for server-wide configuration: organize pattern, review retention days, etc.
   - Writable by admins only.
2. **Per-user UI preferences** — `user_preferences` table, one row per user (`user_id` primary key) with a JSON blob; served by `server/internal/modules/users` (`preferences.go`).
