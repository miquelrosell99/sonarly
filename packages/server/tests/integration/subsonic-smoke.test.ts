...
describe('Subsonic smoke', () => {
  ...
  it('returns ok from ping.view', async () => {
    const res = await app.inject({ method: 'GET', url: buildSubsonicUrl(user.username, user.password, '/rest/ping.view?') });
  ...
  it('returns the library music folder', async () => {
    const res = await app.inject({ method: 'GET', url: buildSubsonicUrl(user.username, user.password, '/rest/getMusicFolders.view?') });
    expect(res.statusCode).toBe(200);
    const body = JSON.parse(res.body);
  ...
  it('returns artists grouped by initial', async () => {
    const res = await app.inject({ method: 'GET', url: buildSubsonicUrl(user.username, user.password, '/rest/getArtists.view?') });
    expect(res.statusCode).toBe(200);
    const body = JSON.parse(res.body);
  ...
  it('returns the seeded album with its song', async () => {
    const res = await app.inject({ method: 'GET', url: buildSubsonicUrl(user.username, user.password, `/rest/getAlbum.view?id=${albumId}`) });
    expect(res.statusCode).toBe(200);
    const body = JSON.parse(res.body);
  ...
  it('streams the song with Accept-Ranges', async () => {
    const res = await app.inject({ method: 'GET', url: buildSubsonicUrl(user.username, user.password, `/rest/stream.view?id=${songId}`) });
    expect(res.statusCode).toBe(200);
