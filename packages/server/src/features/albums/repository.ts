...
export function upsertAlbum(db: Database.Database, album: Album): void {
  const stmt = db.prepare(`
    INSERT INTO albums (id, name, artist_id, artist_name, release_type, year, genre, genre_id, cover_art_id, active, catalog_numbers, barcode, asin, musicbrainz_album_id, musicbrainz_release_group_id, musicbrainz_album_artist_ids, original_year, compilation, total_tracks, total_discs)
    VALUES (@id, @name, @artistId, @artistName, @releaseType, @year, @genre, @genreId, @coverArt, @active, @catalogNumbers, @barcode, @asin, @musicBrainzAlbumId, @musicBrainzReleaseGroupId, @musicBrainzAlbumArtistIds, @originalYear, @compilation, @totalTracks, @totalDiscs)
    ON CONFLICT(id) DO UPDATE SET
      name = excluded.name,
      artist_id = excluded.artist_id,
      artist_name = excluded.artist_name,
      release_type = COALESCE(release_type, excluded.release_type),
      year = excluded.year,
